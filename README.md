...
- **检索**：原文必须逐字包含查询词（空格分隔多个词时，每个词都要出现在同一段里）。点击结果，右侧打开原文并高亮关键字；扫描页没有文字层，只能框出整段。
...
检索时直接在片段原文里逐字匹配，按出现次数排序；片段数很多时（几十万）每次约 1 秒。点击结果时在 PDF 上逐字取坐标定位关键字（跨行也能找到）。
...
升级后旧文档里横向（旋转）页面的高亮可能错位，在文件库点"全部重新解析"即可。
